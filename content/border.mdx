...
<div className="flex flex-col items-start space-y-4 my-4 max-w-xs mr-auto border border-gray-200 p-4 ">
  {[
    "C u in three weeks?",
    "Hellooooooo?",
    "I miss u",
    ...
    <div
      key={index}
      className="px-3 py-2 border border-gray-200 bg-gray-200 text-black max-w-fit rounded-2xl rounded-bl-none"
    >
      {message}
    </div>
  ))}
